...
namespace geometricFunction
{
    ...
    bool are_close_to_real(const std::vector<utils::vec<T, D>> &centroids, const utils::mat<T, M, D> &real, const double eps_to_real)
    {
    ...
    template <typename T, const size_t  D>
    bool is_centroid(std::vector<utils::vec<T, D>>& curr_centroids, const utils::vec<T, D>& point, const double eps_clust) {
    ...
    std::vector<utils::vec<T, D>> reduce_to_centroids(utils::mat<T, N, D>& data, const float min_distance) {
